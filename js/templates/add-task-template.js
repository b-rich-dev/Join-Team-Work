...
export function renderTitleInput(task) {
    return `<div class="label-container">
                ...
                <input name="title" class="input-field" type="text" id="title" placeholder="Enter a title" data-event-handle="true" value="${task?.title ? task.title.replace(/"/g, "&quot;") : ""}"/>
...
export function renderDescriptionInput(task) {
    return `<div class="label-container">
                ...
                <div class="textarea-wrapper">
                    <textarea name="task-description" id="task-description" class="task-description-area" placeholder="Enter a Description">${task?.description ? task.description : ""}</textarea>
...
export function renderDueDateInput(task) {
    ...
    return `<div class="label-container">
                ...
                <div class="input-inline">
                    <input name="datepicker" id="datepicker" type="text" placeholder="dd/mm/yyyy" class="input-field" data-event-handle="true" value="${dateValue}"/>
...
export function renderLeftFormFields(task) {
    return `<div class="left-form" id="left-form-add-task">
...
export function renderSubtasksSection(task) {
    let subtasksArr = Array.isArray(task?.totalSubtasks) ? [...task.totalSubtasks] : [];
    return `<div class="label-container">
...
export function renderRightFormFields(task) {
    return `<div class="right-form" id="right-form-add-task">
...
export function renderFormButtons() {
    return `<div class="form-buttons-part" id="form-buttons-part-add-task">
                ...
                <div class="buttons-area" id="buttons-area-add-task">
                    ...
                    ${!arguments[0] ? `<button type="reset" class="clear-btn" data-event-handle="true">
                                            Clear
                                            <svg class="x-icon" xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                                <path d="M12.14 13.4l-4.9 4.9a.95.95 0 0 1-1.4-1.4l4.9-4.9-4.9-4.9a.95.95 0 0 1 1.4-1.4l4.9 4.9 4.9-4.9a.95.95 0 1 1 1.4 1.4l-4.9 4.9 4.9 4.9a.95.95 0 0 1-1.4 1.4l-4.9-4.9z" fill="currentColor" />
                                            </svg>
                                        </button>`: ""}
...
export function getAddTaskFormHTML(task = null) {
    return `<div id="responsive-div-one" class="responsive-div"></div>
...
/** * Applies styles to the hidden date input element.
 * @param {HTMLInputElement} hiddenInput - The hidden input element.
 * @param {HTMLInputElement} visibleInput - The visible input element.
 */
function applyHiddenInputStyles(hiddenInput, visibleInput) {
...
/** * Attaches event listeners to date picker inputs.
 * @param {HTMLInputElement} visibleInput - The visible input element.
 * @param {HTMLInputElement} hiddenInput - The hidden input element.
 * @param {HTMLElement} container - The container element.
 */
function attachDatePickerEvents(visibleInput, hiddenInput, container) {
    visibleInput.addEventListener("focus", () => hiddenInput.showPicker());
    ...
    if (calendarIcon) calendarIcon.addEventListener("click", () => hiddenInput.showPicker());
}
...
export function initDatePicker(container = document) {
    ...
    applyHiddenInputStyles(hiddenInput, visibleInput);
    visibleInput.parentNode.style.position = "relative";
    visibleInput.parentNode.appendChild(hiddenInput);
    attachDatePickerEvents(visibleInput, hiddenInput, container);
}
...
export function getCategoryOptions() {
    return `<div tabindex="0" class="option" data-category="Technical Task">Technical Task</div>
...
export function renderAssignedToContacts(id, name, initials, avatarColor, avatarImage = null) {
    ...
    return `<div tabindex="0" class="contact-option ${isSelected ? "assigned" : ""}" data-id="${id}" data-name="${name}" data-initials="${initials}" data-avatar-color="${avatarColor}" data-avatar-image="${avatarImage || ''}">
